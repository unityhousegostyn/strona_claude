  ...
  return (
    <div className="max-w-xl space-y-6">
      <div className="flex items-center gap-3">
        <Link href="/admin/users" className="text-sm text-[#6b9478] hover:text-[#6b9478]">← Użytkownicy</Link>
      </div>

      <h2 className="text-2xl font-bold text-[#ecfdf5]">Edytuj użytkownika</h2>

      <div className="bg-[#121c15] border border-[#1e3324] rounded-xl p-5 space-y-2">
        <h3 className="text-xs font-semibold text-[#6b9478] uppercase tracking-wide mb-3">Informacje o koncie</h3>
        <div className="grid grid-cols-2 gap-y-2 text-sm">
          <span className="text-[#4d7a5f]">Email</span>
          <span className="text-[#ecfdf5] font-medium">{profile.email}</span>
          ...
          <span className="text-[#4d7a5f]">Dołączył</span>
          <span className="text-[#6b9478]">{new Date(profile.created_at).toLocaleDateString('pl-PL')}</span>
        </div>
      </div>
