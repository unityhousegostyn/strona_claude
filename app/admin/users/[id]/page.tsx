  ...
  return (
    <div className="max-w-xl space-y-6">
      <div className="flex items-center gap-3">
        <Link href="/admin/users" className="text-sm text-stone-500 hover:text-stone-500">← Użytkownicy</Link>
      </div>

      <h2 className="text-2xl font-bold text-stone-900">Edytuj użytkownika</h2>

      <div className="bg-stone-100 border border-stone-200 rounded-xl p-5 space-y-2">
        <h3 className="text-xs font-semibold text-stone-500 uppercase tracking-wide mb-3">Informacje o koncie</h3>
        <div className="grid grid-cols-2 gap-y-2 text-sm">
          <span className="text-stone-400">Email</span>
          <span className="text-stone-900 font-medium">{profile.email}</span>
          ...
          <span className="text-stone-400">Dołączył</span>
          <span className="text-stone-500">{new Date(profile.created_at).toLocaleDateString('pl-PL')}</span>
        </div>
      </div>
