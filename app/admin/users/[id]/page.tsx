  ...
  return (
    <div className="max-w-xl space-y-6">
      <div className="flex items-center gap-3">
        <Link href="/admin/users" className="text-sm text-[#0f766e] hover:text-[#0f766e]">← Użytkownicy</Link>
      </div>

      <h2 className="text-2xl font-bold text-[#f0fdfa]">Edytuj użytkownika</h2>

      <div className="bg-[#081918] border border-[#0f2d2a] rounded-xl p-5 space-y-2">
        <h3 className="text-xs font-semibold text-[#0f766e] uppercase tracking-wide mb-3">Informacje o koncie</h3>
        <div className="grid grid-cols-2 gap-y-2 text-sm">
          <span className="text-[#115e59]">Email</span>
          <span className="text-[#f0fdfa] font-medium">{profile.email}</span>
          ...
          <span className="text-[#115e59]">Dołączył</span>
          <span className="text-[#0f766e]">{new Date(profile.created_at).toLocaleDateString('pl-PL')}</span>
        </div>
      </div>
