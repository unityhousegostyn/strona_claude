...
const CATEGORIES = [
  { value: 'manager', label: 'Zarządca', icon: '🏢', color: 'bg-amber-900/40 text-amber-400 border-amber-700' },
  ...
  { value: 'other', label: 'Inny', icon: '📋', color: 'bg-[#2a2218] text-[#7a6a58] border-[#3a2e1e]' },
]
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-[#f0ebe0]">Kontakty</h2>
        {canEdit && (
          <button
        ...
        <div className="bg-[#241e14] border border-[#3a2e1e] rounded-xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            {isSuperAdmin && communities.length > 0 && (
              <div>
                <label className="block text-xs font-medium text-[#7a6a58] mb-1">Wspólnota</label>
            ...
            <div className="sm:col-span-2">
              <label className="block text-xs font-medium text-[#7a6a58] mb-1">Opis / Godziny pracy</label>
          ...
          <div className="flex gap-3 pt-1">
            <button onClick={handleSubmit} disabled={isPending} className="bg-amber-600 text-white text-sm font-semibold px-4 py-2 rounded-lg disabled:opacity-50">
      ...
      {isSuperAdmin && communities.length > 0 && (
        <div className="flex items-center gap-3">
          <label className="text-sm font-medium text-[#7a6a58] whitespace-nowrap">Wspólnota:</label>
          <select
      ...
      <div className="flex gap-2 flex-wrap">
        <button
          ...
          className={`px-3 py-1.5 rounded-lg text-xs font-medium transition border ${
            filterCategory === 'all' ? 'bg-amber-700 text-white border-amber-600' : 'bg-[#241e14] text-[#6a5a48] border-[#3a2e1e] hover:border-[#3a2e1e]'
          }`}
        >
          Wszystkie
        </button>
        {CATEGORIES.map(cat => (
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition border ${
              filterCategory === cat.value ? cat.color : 'bg-[#241e14] text-[#6a5a48] border-[#3a2e1e] hover:border-[#3a2e1e]'
            }`}
          >
            {cat.icon} {cat.label}
          </button>
      ...
      {grouped.map(group => (
        <div key={group.value} className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-[#7a6a58] uppercase tracking-wider">{group.label}</h3>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {group.items.map(contact => (
              <div key={contact.id} className={`bg-[#241e14] border rounded-xl p-4 space-y-2 ${group.color.includes('border') ? `border-${group.color.split('border-')[1]}` : 'border-[#3a2e1e]'}`}>
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <p className="font-semibold text-[#f0ebe0]">{contact.name}</p>
                    <p className="text-xs text-[#6a5a48] mt-0.5">{contact.role}</p>
                    {isSuperAdmin && contact.communityName && (
                      <span className="text-xs bg-[#2a2218] text-[#7a6a58] px-1.5 py-0.5 rounded mt-1 inline-block">{contact.communityName}</span>
                    )}
                  </div>
                  {canEdit && (
                    <button
                      ...
                      disabled={isPending}
                      className="text-xs text-[#6a5a48] hover:text-red-400 transition disabled:opacity-50 flex-shrink-0"
                    >
                      ✕
                    </button>
                  )}
                ...
                <div className="space-y-1">
                  {contact.phone && (
                    <a href={`tel:${contact.phone}`} className="flex items-center gap-2 text-sm text-amber-400 hover:text-amber-300 transition">
                      <span>📞</span> {contact.phone}
                    </a>
                  )}
                  {contact.email && (
                    <a href={`mailto:${contact.email}`} className="flex items-center gap-2 text-sm text-amber-400 hover:text-amber-300 transition">
                      <span>✉️</span> {contact.email}
                    </a>
                  )}
                  {contact.description && (
                    <p className="text-xs text-[#6a5a48] pt-1">{contact.description}</p>
                  )}
                </div>
              </div>
            ))}
