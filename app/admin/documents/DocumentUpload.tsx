    ...
    <div className="space-y-3">
      {!showPanel ? (
        ...
        <div className="bg-[#1e1409] border border-[#33200d] rounded-xl p-5 space-y-4">
          <h3 className="font-semibold text-[#fef3c7] text-sm">Nowy dokument</h3>
          ...
          {isSuperAdmin && (
            <div>
              <label className="block text-sm font-medium text-[#fde68a] mb-1">Zasięg</label>
          ...
          {isSuperAdmin && target === 'selected' && (
            <div>
              <label className="block text-sm font-medium text-[#fde68a] mb-1">Wybierz wspólnoty</label>
              <div className="space-y-2 max-h-40 overflow-y-auto border border-[#33200d] rounded-lg p-3">
                {communities.map((c) => (
                  <label key={c.id} className="flex items-center gap-2 cursor-pointer text-sm text-[#fde68a]">
                    <input
                      ...
                      className="rounded border-[#33200d] text-amber-500 focus:ring-amber-400"
                    />
                    {c.name}
                  </label>
                ))}
