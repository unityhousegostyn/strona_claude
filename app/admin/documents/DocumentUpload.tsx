    ...
    <div className="space-y-3">
      {!showPanel ? (
        ...
        <div className="bg-[#081918] border border-[#0f2d2a] rounded-xl p-5 space-y-4">
          <h3 className="font-semibold text-[#ccfbf1] text-sm">Nowy dokument</h3>
          ...
          {isSuperAdmin && (
            <div>
              <label className="block text-sm font-medium text-[#99f6e4] mb-1">Zasięg</label>
          ...
          {isSuperAdmin && target === 'selected' && (
            <div>
              <label className="block text-sm font-medium text-[#99f6e4] mb-1">Wybierz wspólnoty</label>
              <div className="space-y-2 max-h-40 overflow-y-auto border border-[#0f2d2a] rounded-lg p-3">
                {communities.map((c) => (
                  <label key={c.id} className="flex items-center gap-2 cursor-pointer text-sm text-[#99f6e4]">
                    <input
                      ...
                      className="rounded border-[#0f2d2a] text-teal-500 focus:ring-teal-400"
                    />
                    {c.name}
                  </label>
                ))}
