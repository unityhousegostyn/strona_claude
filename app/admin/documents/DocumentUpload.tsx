    ...
    <div className="space-y-3">
      {!showPanel ? (
        ...
        <div className="bg-stone-100 border border-stone-200 rounded-xl p-5 space-y-4">
          <h3 className="font-semibold text-stone-800 text-sm">Nowy dokument</h3>
          ...
          {isSuperAdmin && (
            <div>
              <label className="block text-sm font-medium text-stone-700 mb-1">Zasięg</label>
          ...
          {isSuperAdmin && target === 'selected' && (
            <div>
              <label className="block text-sm font-medium text-stone-700 mb-1">Wybierz wspólnoty</label>
                ...
                {communities.map((c) => (
                  <label key={c.id} className="flex items-center gap-2 cursor-pointer text-sm text-stone-700">
                    <input
                      ...
                      className="rounded border-stone-200 text-green-600 focus:ring-green-400"
                    />
                    {c.name}
                  </label>
                ))}
