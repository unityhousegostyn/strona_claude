    ...
    <div className="space-y-6">
      ...
      {active.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          <div className="bg-teal-950/30 border border-teal-800/40 rounded-xl p-4">
            <p className="text-xs text-[#0f766e] mb-1">Środki zablokowane</p>
            <p className="text-2xl font-bold text-teal-400 tabular-nums">{pln(totalActive)}</p>
            <p className="text-xs text-[#115e59] mt-1">{active.length} aktywna(-ych)</p>
          </div>
          {active.map(d => {
            ...
            return (
              <div key={d.id} className={`rounded-xl p-4 border ${expired ? 'bg-red-950/20 border-red-900/40' : urgent ? 'bg-yellow-950/20 border-yellow-800/40' : 'bg-[#081918] border-[#0f2d2a]'}`}>
                <p className="text-xs font-medium text-[#99f6e4] truncate">{d.bank_name ?? typeLabel[d.type]}</p>
                <p className="text-lg font-bold text-[#f0fdfa] tabular-nums mt-1">{pln(d.amount)}</p>
                {d.interest_rate && <p className="text-xs text-teal-500 mt-0.5">{d.interest_rate}% / rok</p>}
                {d.end_date && (
                  <p className={`text-xs mt-0.5 ${expired ? 'text-red-400 font-semibold' : urgent ? 'text-yellow-400' : 'text-[#115e59]'}`}>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-xs font-semibold text-[#115e59] uppercase tracking-widest">Aktywne lokaty i konta</h3>
          <button
            onClick={() => { setShowForm(v => !v); if (showForm) resetForm() }}
            className="flex items-center gap-1.5 text-sm font-semibold text-teal-500 hover:text-teal-400 transition"
          >
        ...
        {/* Formularz */}
        {showForm && (
          <div className="bg-[#081918] border border-teal-800/40 rounded-xl p-5 mb-4 space-y-4">
            <h4 className="text-sm font-semibold text-[#f0fdfa]">Nowa lokata / konto oszczędnościowe</h4>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {isSuperAdmin && communities.length > 1 && (
                <div className="sm:col-span-2">
                  <label className="block text-xs font-semibold text-[#0f766e] mb-1.5 uppercase tracking-wide">Wspólnota</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-[#0f766e] mb-1.5 uppercase tracking-wide">Typ</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-[#0f766e] mb-1.5 uppercase tracking-wide">Bank</label>
                <input value={bankName} onChange={e => setBankName(e.target.value)} placeholder="np. PKO BP, Santander…" className="input w-full" />
              </div>

              <div>
                <label className="block text-xs font-semibold text-[#0f766e] mb-1.5 uppercase tracking-wide">Kwota (zł) *</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-[#0f766e] mb-1.5 uppercase tracking-wide">Oprocentowanie (% / rok)</label>
                <input value={rate} onChange={e => setRate(e.target.value)} placeholder="np. 5.5" type="number" min="0" step="0.01" className="input w-full" />
              </div>

              <div>
                <label className="block text-xs font-semibold text-[#0f766e] mb-1.5 uppercase tracking-wide">Data założenia *</label>
                <input value={startDate} onChange={e => setStartDate(e.target.value)} type="date" className="input w-full" />
              </div>

              <div>
                <label className="block text-xs font-semibold text-[#0f766e] mb-1.5 uppercase tracking-wide">Termin (data zakończenia)</label>
                ...
                <p className="text-xs text-[#133835] mt-1">Zostaw puste dla konta bez określonego terminu</p>
              </div>

              <div className="sm:col-span-2">
                <label className="block text-xs font-semibold text-[#0f766e] mb-1.5 uppercase tracking-wide">Opis / notatka</label>
            ...
            {(() => {
              ...
              if (!calc) return null
              return (
                <div className="bg-[#051210] border border-[#0f2d2a] rounded-xl p-4">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="text-center">
                      <p className="text-[10px] text-[#115e59] mb-1">Odsetki brutto</p>
                      <p className="text-base font-bold text-[#99f6e4] tabular-nums">{pln(calc.gross)}</p>
                    </div>
                    <div className="text-center">
                      <p className="text-[10px] text-[#115e59] mb-1">Podatek Belki (19%)</p>
                  ...
                  {!endDate && (
                    <p className="text-[10px] text-[#133835] mt-2">* Wyliczenie dla 1 roku. Ustaw datę zakończenia, aby zobaczyć dokładną kwotę.</p>
                  )}
                </div>
              )
            })()}
            ...
            <div className="flex justify-end">
              <button
                onClick={handleAdd}
                disabled={saving}
                className="flex items-center gap-2 bg-teal-700 hover:bg-teal-600 text-white font-semibold px-5 py-2.5 rounded-lg text-sm transition disabled:opacity-40"
              >
        ...
        {active.length === 0 && !showForm ? (
          <div className="bg-[#081918] border border-dashed border-[#0f2d2a] rounded-xl p-8 text-center">
            <p className="text-4xl mb-3">🏦</p>
            <p className="text-sm text-[#0f766e]">Brak aktywnych lokat ani kont oszczędnościowych.</p>
            <button onClick={() => setShowForm(true)} className="mt-3 text-sm text-teal-500 hover:underline">
      ...
      {closed.length > 0 && (
        <div>
          <h3 className="text-xs font-semibold text-[#115e59] uppercase tracking-widest mb-3">Zakończone</h3>
...
function DepositRow({
  ...
  return (
    <div className={`bg-[#081918] border rounded-xl px-4 py-3.5 ${expired ? 'border-red-900/50' : urgent ? 'border-yellow-800/50' : 'border-[#0f2d2a]'}`}>
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <div>
              <p className="text-sm font-semibold text-[#f0fdfa]">
                ...
                {d.status === 'closed' && <span className="ml-2 text-xs text-[#115e59] font-normal">(zakończona)</span>}
              </p>
              {d.description && <p className="text-xs text-[#0f766e] mt-0.5 truncate">{d.description}</p>}
              {showComm && <p className="text-xs text-[#133835] mt-0.5">{commMap[d.community_id] ?? '—'}</p>}
            </div>
            <p className="text-base font-bold text-teal-400 tabular-nums flex-shrink-0">{pln(d.amount)}</p>
          </div>

          <div className="flex items-center gap-3 mt-2 flex-wrap">
            {d.interest_rate != null && (
              <span className="text-xs text-teal-500 font-semibold">{d.interest_rate}% / rok</span>
            )}
            <span className="text-xs text-[#115e59]">od {new Date(d.start_date).toLocaleDateString('pl-PL')}</span>
            {d.end_date && (
              <span className={`text-xs font-medium ${expired ? 'text-red-400' : urgent ? 'text-yellow-400' : 'text-[#0f766e]'}`}>
          ...
          {calc && (
            <div className="flex items-center gap-3 mt-2 pt-2 border-t border-[#0c2220] flex-wrap">
              <span className="text-[10px] text-[#115e59]">
                brutto <span className="text-[#99f6e4] font-semibold">{pln(calc.gross)}</span>
              </span>
              <span className="text-[10px] text-[#133835]">−</span>
              <span className="text-[10px] text-[#115e59]">
              ...
              {!d.end_date && <span className="text-[10px] text-[#0f2d2a]">(szac. za 1 rok)</span>}
            </div>
          )}
        </div>
      </div>
      ...
      {/* Przyciski akcji */}
      {d.status === 'active' && (
        <div className="flex items-center gap-2 mt-3 pt-3 border-t border-[#0c2220]">
          {/* Zakończ i zaksięguj */}
          <button
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg text-[#115e59] hover:text-red-400 hover:bg-red-950/20 border border-transparent hover:border-red-900/30 transition"
