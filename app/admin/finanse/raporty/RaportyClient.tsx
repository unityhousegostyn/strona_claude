  ...
  return (
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3 print:hidden">
        <div>
          <h2 className="text-2xl font-bold text-stone-900">📊 Raporty finansowe</h2>
          <p className="text-sm text-stone-400 mt-0.5">Dokumentacja zgodna z UoWL i KC</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {REPORTS.map(r => (
            <button
              ...
              className="text-left bg-stone-100 border border-stone-200 hover:border-stone-300 rounded-xl p-5 transition group"
            >
              <div className="flex items-start gap-4">
                <span className="text-3xl">{r.icon}</span>
                <div className="flex-1">
                  <p className="font-semibold text-stone-900 group-hover:text-white">{r.title}</p>
                  <p className="text-xs text-stone-400 mt-0.5">{r.subtitle}</p>
                  <span className="inline-block mt-2 text-xs bg-green-950/50 text-green-400 px-2 py-0.5 rounded-full">{r.art}</span>
                </div>
                <svg className="w-4 h-4 text-stone-400 group-hover:text-stone-500 mt-1 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {activeReport && (
        <div>
          ...
          <div className="flex items-center gap-3 mb-5 print:hidden">
            <button onClick={() => setActiveReport(null)} className="flex items-center gap-2 text-sm text-stone-500 hover:text-stone-900 transition">
            ...
            <div className="flex-1" />
            <button onClick={handleExportExcel} className="flex items-center gap-2 text-sm bg-stone-200 hover:bg-stone-300 text-stone-800 px-4 py-2 rounded-lg transition">
              📊 Eksport Excel
            </button>
            <button onClick={handlePrint} className="flex items-center gap-2 text-sm bg-stone-200 hover:bg-stone-300 text-stone-800 px-4 py-2 rounded-lg transition">
              🖨️ Drukuj / zapisz PDF
            </button>
          </div>
          ...
          <div ref={reportRef} className="print-area">
              ...
              <div className="space-y-6">
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-stone-200">
                          <th className="text-left py-2 pr-4 text-stone-500 font-medium">Miesiąc</th>
                          <th className="text-right py-2 px-3 text-stone-500 font-medium">Wpłaty</th>
                          <th className="text-right py-2 px-3 text-stone-500 font-medium">Inne przychody</th>
                          <th className="text-right py-2 px-3 text-stone-500 font-medium">Koszty</th>
                          <th className="text-right py-2 pl-3 text-stone-500 font-medium">Saldo</th>
                        </tr>
                      </thead>
                      <tbody>
                        {Array.from({ length: 12 }, (_, i) => i + 1).map(m => {
                          ...
                          return (
                            <tr key={m} className={`border-b border-stone-200/50 ${!hasData ? 'opacity-40' : ''}`}>
                              <td className="py-2 pr-4 text-stone-700">{MONTHS_FULL[m - 1]}</td>
                              <td className="text-right py-2 px-3 text-stone-800">{monthlyPaid[m] > 0 ? pln(monthlyPaid[m]) : '—'}</td>
                              <td className="text-right py-2 px-3 text-stone-800">{monthlyOtherIncome[m] > 0 ? pln(monthlyOtherIncome[m]) : '—'}</td>
                              <td className="text-right py-2 px-3 text-stone-800">{exp > 0 ? pln(exp) : '—'}</td>
                      ...
                      <tfoot>
                        <tr className="border-t border-stone-300">
                          <td className="py-3 pr-4 font-bold text-stone-900">RAZEM</td>
                ...
                <ReportSection title="Koszty według kategorii">
                  {(() => {
                    ...
                    if (sorted.length === 0) return <p className="text-stone-400 text-sm">Brak kosztów w tym roku.</p>
                    return (
                      <table className="w-full text-sm">
                        <thead><tr className="border-b border-stone-200"><th className="text-left py-2 text-stone-500 font-medium">Kategoria</th><th className="text-right py-2 text-stone-500 font-medium">Kwota</th><th className="text-right py-2 text-stone-500 font-medium">%</th></tr></thead>
                        <tbody>
                          {sorted.map(([cat, amt]) => (
                            <tr key={cat} className="border-b border-stone-200/50">
                              <td className="py-2 text-stone-700">{EXP_CAT_LABELS[cat] ?? cat}</td>
                              <td className="text-right py-2 text-stone-800">{pln(amt)}</td>
                              <td className="text-right py-2 text-stone-500">{totalExpenses > 0 ? Math.round(amt / totalExpenses * 100) : 0}%</td>
                            </tr>
                          ))}
                        </tbody>
                        <tfoot><tr className="border-t border-stone-300"><td className="py-2 font-bold text-stone-900">Razem</td><td className="text-right py-2 font-bold text-red-400">{pln(totalExpenses)}</td><td /></tr></tfoot>
                      </table>
                    )
                  })()}
                </ReportSection>
              ...
              <div className="space-y-6">
                ...
                <ReportSection title={`Rozliczenie per lokal (${commApts.length} lokali)`}>
                  {aptReconciliation.length === 0
                    ? <p className="text-stone-400 text-sm">Brak danych dla wybranej wspólnoty i roku.</p>
                    : <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b border-stone-200">
                            <th className="text-left py-2 pr-3 text-stone-500 font-medium">Lokal</th>
                            <th className="text-left py-2 pr-3 text-stone-500 font-medium">Właściciel</th>
                            <th className="text-right py-2 px-2 text-stone-500 font-medium">m²</th>
                            <th className="text-right py-2 px-2 text-stone-500 font-medium">Naliczono</th>
                            <th className="text-right py-2 px-2 text-stone-500 font-medium">Wpłacono</th>
                            <th className="text-right py-2 pl-2 text-stone-500 font-medium">Saldo</th>
                          </tr>
                        </thead>
                        <tbody>
                          {aptReconciliation.map(({ apt, paid, charged, balance }) => (
                            <tr key={apt.id} className="border-b border-stone-200/50 hover:bg-stone-200/20">
                              <td className="py-2 pr-3 font-mono text-stone-800">{apt.number}</td>
                              <td className="py-2 pr-3 text-stone-700 truncate max-w-[160px]">{apt.owner_name}</td>
                              <td className="text-right py-2 px-2 text-stone-500">{apt.area_m2}</td>
                              <td className="text-right py-2 px-2 text-stone-700">{charged > 0 ? pln(charged) : '—'}</td>
                              <td className="text-right py-2 px-2 text-stone-700">{paid > 0 ? pln(paid) : '—'}</td>
                              <td className={`text-right py-2 pl-2 font-semibold ${balance > 0.01 ? 'text-green-400' : balance < -0.01 ? 'text-red-400' : 'text-stone-400'}`}>
                        ...
                        <tfoot>
                          <tr className="border-t border-stone-300">
                            <td colSpan={3} className="py-3 font-bold text-stone-900">RAZEM</td>
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b border-stone-200">
                            ...
                            <th className="text-right py-2 px-2 text-stone-500 font-medium">Naliczono</th>
                            <th className="text-right py-2 px-2 text-stone-500 font-medium">Wpłacono</th>
                            <th className="text-right py-2 pl-2 text-stone-500 font-medium">Zaległość</th>
                          </tr>
                        </thead>
                        <tbody>
                          {debtors.map(({ apt, paid, charged, balance }) => (
                            <tr key={apt.id} className="border-b border-stone-200/50">
                              <td className="py-2 pr-3 font-mono text-stone-800">{apt.number}</td>
                              <td className="py-2 pr-3 text-stone-700">{apt.owner_name}</td>
                              <td className="text-right py-2 px-2 text-stone-700">{pln(charged)}</td>
                              <td className="text-right py-2 px-2 text-stone-700">{pln(paid)}</td>
                        ...
                        <tfoot>
                          <tr className="border-t border-stone-300">
                            <td colSpan={4} className="py-3 font-bold text-stone-900">Łącznie zaległości</td>
            ...
            {activeReport === 'plan' && (
              <div className="space-y-6">
                ...
                {!latestRate
                  ? <div className="text-center py-12 text-stone-400"><p className="text-3xl mb-3">⚙️</p><p>Brak zdefiniowanych stawek dla tej wspólnoty.</p></div>
                  : <>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    ...
                    <ReportSection title="Porównanie per kategoria">
                      <table className="w-full text-sm">
                        <thead><tr className="border-b border-stone-200">
                          <th className="text-left py-2 pr-4 text-stone-500 font-medium">Kategoria</th>
                          <th className="text-right py-2 px-3 text-stone-500 font-medium">Plan</th>
                          <th className="text-right py-2 px-3 text-stone-500 font-medium">Wykonanie</th>
                          <th className="text-right py-2 pl-3 text-stone-500 font-medium">Różnica</th>
                        </tr></thead>
                        <tbody>
                          {Object.keys({ ...planByCategory, ...executionByCategory }).sort().map(cat => {
                            ...
                            return (
                              <tr key={cat} className="border-b border-stone-200/50">
                                <td className="py-2 pr-4 text-stone-700">{EXP_CAT_LABELS[cat] ?? cat}</td>
                                <td className="text-right py-2 px-3 text-stone-700">{plan > 0 ? pln(plan) : '—'}</td>
                                <td className="text-right py-2 px-3 text-stone-700">{exec > 0 ? pln(exec) : '—'}</td>
                                <td className={`text-right py-2 pl-3 font-medium ${diff > 0 ? 'text-red-400' : diff < 0 ? 'text-green-400' : 'text-stone-400'}`}>
              ...
              <div className="space-y-6">
                ...
                <ReportSection title="Historia funduszu remontowego">
                  {renovFundCumulative.length === 0
                    ? <p className="text-stone-400 text-sm">Brak danych.</p>
                    : <table className="w-full text-sm">
                      <thead><tr className="border-b border-stone-200">
                        <th className="text-left py-2 pr-4 text-stone-500 font-medium">Rok</th>
                        <th className="text-right py-2 px-3 text-stone-500 font-medium">Naliczenia</th>
                        <th className="text-right py-2 px-3 text-stone-500 font-medium">Wydatki na remonty</th>
                        <th className="text-right py-2 px-3 text-stone-500 font-medium">Saldo roku</th>
                        <th className="text-right py-2 pl-3 text-stone-500 font-medium">Saldo skum.</th>
                      </tr></thead>
                      <tbody>
                        {renovFundCumulative.map(r => (
                          <tr key={r.year} className={`border-b border-stone-200/50 ${r.year === filterYear ? 'bg-green-950/10' : ''}`}>
                            <td className="py-2 pr-4 text-stone-700 font-medium">{r.year}{r.year === filterYear && <span className="ml-2 text-xs text-green-400">(bieżący)</span>}</td>
                            <td className="text-right py-2 px-3 text-stone-700">{pln(r.naliczenia)}</td>
                            <td className="text-right py-2 px-3 text-stone-700">{r.wydatki > 0 ? pln(r.wydatki) : '—'}</td>
                ...
                {(() => {
                  ...
                  if (months.length === 0) return (
                    <div className="bg-stone-100 border border-stone-200 rounded-xl p-5">
                      <p className="text-stone-400 text-sm">Brak kosztów w roku {filterYear}.</p>
                    </div>
                  )
                  let grandTotal = 0
                  return (
                    <div className="space-y-4">
                          ...
                          <ReportSection key={m} title={`${MONTHS_FULL[m - 1]} ${filterYear}`}>
                            <div className="overflow-x-auto">
                              <table className="w-full text-sm">
                                <thead>
                                  <tr className="border-b border-stone-200">
                                    <th className="text-left py-2 pr-3 text-stone-500 font-medium w-24">Data</th>
                                    <th className="text-left py-2 pr-3 text-stone-500 font-medium">Od kogo / opis</th>
                                    <th className="text-left py-2 pr-3 text-stone-500 font-medium w-32">Kategoria</th>
                                    <th className="text-left py-2 pr-3 text-stone-500 font-medium w-28">Nr faktury</th>
                                    <th className="text-right py-2 text-stone-500 font-medium w-28">Kwota</th>
                                  </tr>
                                </thead>
                                <tbody>
                                  {rows.map((e, i) => (
                                    <tr key={i} className="border-b border-stone-200/50 hover:bg-stone-200/20">
                                      <td className="py-2 pr-3 text-stone-500 text-xs">{e.expense_date}</td>
                                      <td className="py-2 pr-3 text-stone-800">{e.description}</td>
                                      <td className="py-2 pr-3 text-stone-500 text-xs">{EXP_CAT_LABELS[e.category] ?? e.category}</td>
                                      <td className="py-2 pr-3 text-stone-500 text-xs font-mono">{e.invoice_number ?? '—'}</td>
                                      <td className="text-right py-2 text-stone-800 font-medium">{pln(e.amount)}</td>
                                    </tr>
                                  ))}
                                </tbody>
                                <tfoot>
                                  <tr className="border-t border-stone-300">
                                    <td colSpan={4} className="py-2 pr-3 text-sm font-semibold text-stone-700">Razem {MONTHS_SHORT[m - 1]}</td>
                      ...
                      {/* Grand total */}
                      <div className="bg-stone-200/60 border border-stone-200 rounded-xl p-4 flex items-center justify-between">
                        <span className="font-bold text-stone-900 text-sm">SUMA ROCZNA {filterYear}</span>
...
function ReportHeader({ title, subtitle, art }: { title: string; subtitle: string; art: string }) {
  return (
    <div className="border-b border-stone-200 pb-4">
      ...
      <p className="text-xs text-stone-400 mt-2">Wygenerowano: {new Date().toLocaleDateString('pl-PL')}</p>
    </div>
  )
}
...
function ReportSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="bg-stone-100 border border-stone-200 rounded-xl p-5">
      <h4 className="text-sm font-semibold text-stone-500 mb-4">{title}</h4>
      {children}
    </div>
  )
}
...
function LegalFooter({ text }: { text: string }) {
  return (
    <div className="border-t border-stone-200 pt-4">
      <p className="text-xs text-stone-400 leading-relaxed">⚖️ <em>{text}</em></p>
    </div>
  )
}
