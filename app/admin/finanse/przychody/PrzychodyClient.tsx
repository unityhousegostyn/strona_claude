...
export default function PrzychodyClient({ incomeEntries, settlementsMap, communities, commMap, isSuperAdmin, defaultCommunityId, categories, currentYear }: Props) {
  ...
  const catColors: Record<string, string> = { odsetki:'bg-yellow-950/40 text-yellow-400', zwrot:'bg-emerald-950/40 text-emerald-400', dotacja:'bg-emerald-950/40 text-emerald-400', inne:'bg-[#162418] text-[#6b9478]' }
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-2xl font-bold text-[#ecfdf5]">💰 Przychody wspólnoty</h2>
          <p className="text-sm text-[#4d7a5f] mt-0.5">Odsetki od lokat, zwroty, dotacje i inne dochody</p>
        </div>
        <div className="flex gap-2">
          ...
          <button onClick={() => setShowForm(!showForm)} className="bg-emerald-700 hover:bg-emerald-600 text-white text-sm font-semibold px-4 py-2 rounded-lg transition">+ Dodaj przychód</button>
        </div>
      </div>

      {showForm && (
        <div className="bg-[#121c15] border border-[#1e3324] rounded-xl p-5 space-y-4">
          <h3 className="font-semibold text-[#d1fae5]">Nowy przychód</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div><label className="text-xs text-[#6b9478] block mb-1">Kategoria *</label><select className="input w-full" value={form.category} onChange={e=>setForm(p=>({...p,category:e.target.value as IncomeCategory}))}>{categories.map(c=><option key={c.value} value={c.value}>{c.label}</option>)}</select></div>
            <div className="sm:col-span-2"><label className="text-xs text-[#6b9478] block mb-1">Opis *</label><input className="input w-full" placeholder="np. Odsetki od lokaty bankowej" value={form.description} onChange={e=>setForm(p=>({...p,description:e.target.value}))} /></div>
            ...
            <div><label className="text-xs text-[#6b9478] block mb-1">Data *</label><input className="input w-full" type="date" value={form.income_date} onChange={e=>setForm(p=>({...p,income_date:e.target.value}))} /></div>
          </div>
          {formError && <p className="text-sm text-red-400">{formError}</p>}
          <div className="flex gap-3">
            <button onClick={handleAdd} disabled={isPending} className="bg-emerald-700 text-white text-sm font-semibold px-5 py-2 rounded-lg disabled:opacity-50">{isPending ? 'Zapisuję...' : 'Dodaj przychód'}</button>
      ...
      <div className="bg-[#121c15] border border-[#1e3324] rounded-xl p-4 space-y-3">
        <div><p className="text-sm font-semibold text-[#a7f3d0]">📥 Import przychodów z CSV</p><p className="text-xs text-[#4d7a5f] mt-0.5">Format: <code className="text-[#6b9478]">data;opis;kategoria;kwota</code> (kategorie: odsetki, zwrot, dotacja, inne)</p></div>
        ...
        <div onDragOver={e=>{e.preventDefault();setCsvDragOver(true)}} onDragLeave={()=>setCsvDragOver(false)} onDrop={e=>{e.preventDefault();setCsvDragOver(false);const f=e.dataTransfer.files[0];if(f)handleCsvFile(f)}} onClick={()=>fileRef.current?.click()} className={`cursor-pointer rounded-xl border-2 border-dashed transition-all py-8 px-4 text-center select-none ${csvDragOver?'border-green-500 bg-emerald-950/20':csvFileName?'border-green-700 bg-emerald-950/20':'border-[#1e3324] bg-[#162418]/30 hover:border-[#2a4a2a]'}`}>
        ...
        <a href={`data:text/plain;charset=utf-8,${encodeURIComponent('data;opis;kategoria;kwota\n2026-01-15;Odsetki od lokaty PKO BP;odsetki;376.88\n2026-03-15;Odsetki od lokaty PKO BP;odsetki;423.45')}`} download="szablon_przychodow.csv" className="text-xs text-emerald-400 hover:text-emerald-300 underline">Pobierz szablon</a>
      ...
      {tab==='summary'?(
        <div className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-[#121c15] border border-[#1e3324] rounded-xl p-4 text-center"><p className="text-xs text-[#4d7a5f] mb-1">Wpłaty mieszkańców</p><p className="text-2xl font-bold text-emerald-400">{pln(totalSettlements)}</p></div>
            <div className="bg-[#121c15] border border-[#1e3324] rounded-xl p-4 text-center"><p className="text-xs text-[#4d7a5f] mb-1">Inne przychody</p><p className="text-2xl font-bold text-yellow-400">{pln(totalOther)}</p></div>
            <div className="bg-[#121c15] border border-[#1e3324] rounded-xl p-4 text-center"><p className="text-xs text-[#4d7a5f] mb-1">Razem przychody</p><p className="text-2xl font-bold text-emerald-400">{pln(totalAll)}</p></div>
          </div>

          <div className="bg-[#121c15] border border-[#1e3324] rounded-xl p-5">
            <h3 className="text-sm font-semibold text-[#6b9478] mb-1">Przychody miesięcznie — {filterYear}</h3>
            <div className="flex gap-4 text-xs text-[#4d7a5f] mb-4">
              <span><span className="inline-block w-3 h-3 rounded-sm bg-emerald-600/60 mr-1"/>Wpłaty mieszkańców</span>
              <span><span className="inline-block w-3 h-3 rounded-sm bg-yellow-500/60 mr-1"/>Inne przychody</span>
            </div>
            <div className="flex items-end gap-1 h-32">
              {MONTHS.map((name,idx)=>{const m=idx+1;const sett=monthlySettlements[m]??0;const oth=monthlyOther[m]??0;const tot=sett+oth;const hS=Math.max(oth>0?2:0,Math.round((sett/maxBar)*120));const hO=Math.max(oth>0?2:0,Math.round((oth/maxBar)*120));return(<div key={m} className="flex-1 flex flex-col items-center gap-0.5"><div className="flex flex-col justify-end w-full" style={{height:120}}><div title={`Inne: ${pln(oth)}`} style={{height:hO}} className="bg-yellow-500/60 rounded-t-sm"/><div title={`Wpłaty: ${pln(sett)}`} style={{height:hS}} className="bg-emerald-600/60"/></div><span className="text-xs text-[#4d7a5f]">{name}</span></div>)})}
            </div>
          </div>

          {filtered.length > 0 && (
            <div className="bg-[#121c15] border border-[#1e3324] rounded-xl p-5">
              <h3 className="text-sm font-semibold text-[#6b9478] mb-4">Inne przychody według kategorii</h3>
              {(() => {
                ...
                return Object.entries(byCat).sort((a,b)=>b[1]-a[1]).map(([cat,amt])=>{
                  ...
                  return (<div key={cat} className="flex items-center gap-3 mb-3"><span className={`text-xs px-2 py-0.5 rounded-full w-32 text-center flex-shrink-0 ${catColors[cat]??catColors.inne}`}>{catLabel(cat)}</span><div className="flex-1 bg-[#162418] rounded-full h-2"><div className="bg-yellow-500 h-2 rounded-full" style={{width:`${pct}%`}}/></div><span className="text-sm font-semibold text-[#d1fae5] w-28 text-right">{pln(amt)}</span></div>)
                })
              })()}
            </div>
          )}
        ...
        <div>
          {filtered.length === 0 ? (
            <div className="text-center py-16 text-[#4d7a5f]"><p className="text-3xl mb-3">💰</p><p>Brak przychodów za {filterYear} rok.</p><button onClick={() => setShowForm(true)} className="mt-4 text-sm text-emerald-400 hover:underline">+ Dodaj pierwszy przychód</button></div>
          ) : (
            <div className="space-y-2">
              {paginatedList.map(e => editId === e.id ? (
                <div key={e.id} className="bg-[#121c15] border border-emerald-700 rounded-xl p-4 space-y-3">
                  ...
                  <div className="flex gap-2">
                    <button onClick={handleUpdate} disabled={isPending} className="bg-emerald-700 text-white text-xs font-semibold px-4 py-1.5 rounded-lg disabled:opacity-50">{isPending ? 'Zapisuję...' : 'Zapisz'}</button>
                    <button onClick={() => setEditId(null)} className="text-xs text-[#4d7a5f] hover:text-[#a7f3d0]">Anuluj</button>
                  </div>
                </div>
              ) : (
                <div key={e.id} className="bg-[#121c15] border border-[#1e3324] rounded-xl px-4 py-3 flex items-center gap-3 flex-wrap hover:border-[#1e3324] transition">
                  ...
                  <div className="flex-1 min-w-0"><p className="text-sm font-medium text-[#d1fae5] truncate">{e.description}</p><p className="text-xs text-[#4d7a5f] mt-0.5">{new Date(e.income_date).toLocaleDateString('pl-PL')}{isSuperAdmin && ` · ${commMap[e.community_id] ?? '—'}`}</p></div>
                  <p className="text-sm font-bold text-emerald-400 flex-shrink-0">{pln(e.amount)}</p>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <button onClick={() => { setEditId(e.id); setEditForm({ category: e.category as IncomeCategory, description: e.description, amount: String(e.amount), income_date: e.income_date }); setEditError(null) }} className="text-xs text-emerald-500 hover:underline">Edytuj</button>
              ...
              <div className="mt-4 pt-4 border-t border-[#1e3324] flex justify-between items-center"><p className="text-sm text-[#4d7a5f]">{filtered.length} pozycji</p><p className="text-base font-bold text-emerald-400">Razem: {pln(totalOther)}</p></div>
            </div>
          )}
        </div>
      )}
