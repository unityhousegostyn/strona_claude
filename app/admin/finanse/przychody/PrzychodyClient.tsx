...
export default function PrzychodyClient({ incomeEntries, settlementsMap, communities, commMap, isSuperAdmin, defaultCommunityId, categories, currentYear }: Props) {
  ...
  const catColors: Record<string, string> = { odsetki:'bg-yellow-950/40 text-yellow-400', zwrot:'bg-green-950/40 text-green-400', dotacja:'bg-green-950/40 text-green-400', inne:'bg-stone-200 text-stone-500' }
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-2xl font-bold text-stone-900">💰 Przychody wspólnoty</h2>
          <p className="text-sm text-stone-400 mt-0.5">Odsetki od lokat, zwroty, dotacje i inne dochody</p>
        </div>
      ...
      {showForm && (
        <div className="bg-stone-100 border border-stone-200 rounded-xl p-5 space-y-4">
          <h3 className="font-semibold text-stone-800">Nowy przychód</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div><label className="text-xs text-stone-500 block mb-1">Kategoria *</label><select className="input w-full" value={form.category} onChange={e=>setForm(p=>({...p,category:e.target.value as IncomeCategory}))}>{categories.map(c=><option key={c.value} value={c.value}>{c.label}</option>)}</select></div>
            <div className="sm:col-span-2"><label className="text-xs text-stone-500 block mb-1">Opis *</label><input className="input w-full" placeholder="np. Odsetki od lokaty bankowej" value={form.description} onChange={e=>setForm(p=>({...p,description:e.target.value}))} /></div>
            ...
            <div><label className="text-xs text-stone-500 block mb-1">Data *</label><input className="input w-full" type="date" value={form.income_date} onChange={e=>setForm(p=>({...p,income_date:e.target.value}))} /></div>
          </div>
      ...
      <div className="bg-stone-100 border border-stone-200 rounded-xl p-4 space-y-3">
        <div><p className="text-sm font-semibold text-stone-700">📥 Import przychodów z CSV</p><p className="text-xs text-stone-400 mt-0.5">Format: <code className="text-stone-500">data;opis;kategoria;kwota</code> (kategorie: odsetki, zwrot, dotacja, inne)</p></div>
        ...
        <div onDragOver={e=>{e.preventDefault();setCsvDragOver(true)}} onDragLeave={()=>setCsvDragOver(false)} onDrop={e=>{e.preventDefault();setCsvDragOver(false);const f=e.dataTransfer.files[0];if(f)handleCsvFile(f)}} onClick={()=>fileRef.current?.click()} className={`cursor-pointer rounded-xl border-2 border-dashed transition-all py-8 px-4 text-center select-none ${csvDragOver?'border-green-500 bg-green-950/20':csvFileName?'border-green-700 bg-green-950/20':'border-stone-200 bg-stone-200/30 hover:border-stone-300'}`}>
      ...
      {tab==='summary'?(
        <div className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-stone-100 border border-stone-200 rounded-xl p-4 text-center"><p className="text-xs text-stone-400 mb-1">Wpłaty mieszkańców</p><p className="text-2xl font-bold text-green-400">{pln(totalSettlements)}</p></div>
            <div className="bg-stone-100 border border-stone-200 rounded-xl p-4 text-center"><p className="text-xs text-stone-400 mb-1">Inne przychody</p><p className="text-2xl font-bold text-yellow-400">{pln(totalOther)}</p></div>
            <div className="bg-stone-100 border border-stone-200 rounded-xl p-4 text-center"><p className="text-xs text-stone-400 mb-1">Razem przychody</p><p className="text-2xl font-bold text-green-400">{pln(totalAll)}</p></div>
          </div>

          <div className="bg-stone-100 border border-stone-200 rounded-xl p-5">
            <h3 className="text-sm font-semibold text-stone-500 mb-1">Przychody miesięcznie — {filterYear}</h3>
            ...
            <div className="bg-stone-100 border border-stone-200 rounded-xl p-5">
              ...
              {(() => {
                ...
                return Object.entries(byCat).sort((a,b)=>b[1]-a[1]).map(([cat,amt])=>{
                  ...
                  return (<div key={cat} className="flex items-center gap-3 mb-3"><span className={`text-xs px-2 py-0.5 rounded-full w-32 text-center flex-shrink-0 ${catColors[cat]??catColors.inne}`}>{catLabel(cat)}</span><div className="flex-1 bg-stone-200 rounded-full h-2"><div className="bg-yellow-500 h-2 rounded-full" style={{width:`${pct}%`}}/></div><span className="text-sm font-semibold text-stone-800 w-28 text-right">{pln(amt)}</span></div>)
                })
              })()}
            </div>
          )}
        ...
        <div>
          {filtered.length === 0 ? (
            <div className="text-center py-16 text-stone-400"><p className="text-3xl mb-3">💰</p><p>Brak przychodów za {filterYear} rok.</p><button onClick={() => setShowForm(true)} className="mt-4 text-sm text-green-400 hover:underline">+ Dodaj pierwszy przychód</button></div>
          ) : (
            <div className="space-y-2">
              {paginatedList.map(e => editId === e.id ? (
                <div key={e.id} className="bg-stone-100 border border-green-800 rounded-xl p-4 space-y-3">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => setEditId(null)} className="text-xs text-stone-400 hover:text-stone-700">Anuluj</button>
                  </div>
                </div>
              ) : (
                <div key={e.id} className="bg-stone-100 border border-stone-200 rounded-xl px-4 py-3 flex items-center gap-3 flex-wrap hover:border-stone-200 transition">
                  ...
                  <div className="flex-1 min-w-0"><p className="text-sm font-medium text-stone-800 truncate">{e.description}</p><p className="text-xs text-stone-400 mt-0.5">{new Date(e.income_date).toLocaleDateString('pl-PL')}{isSuperAdmin && ` · ${commMap[e.community_id] ?? '—'}`}</p></div>
              ...
              <div className="mt-4 pt-4 border-t border-stone-200 flex justify-between items-center"><p className="text-sm text-stone-400">{filtered.length} pozycji</p><p className="text-base font-bold text-green-400">Razem: {pln(totalOther)}</p></div>
            </div>
          )}
        </div>
      )}
