...
export default function PrzychodyClient({ incomeEntries, settlementsMap, communities, commMap, isSuperAdmin, defaultCommunityId, categories, currentYear }: Props) {
  ...
  const catColors: Record<string, string> = { odsetki:'bg-yellow-950/40 text-yellow-400', zwrot:'bg-amber-950/40 text-amber-400', dotacja:'bg-amber-950/40 text-amber-400', inne:'bg-[#2a2218] text-[#7a6a58]' }
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-2xl font-bold text-[#f0ebe0]">💰 Przychody wspólnoty</h2>
          <p className="text-sm text-[#6a5a48] mt-0.5">Odsetki od lokat, zwroty, dotacje i inne dochody</p>
        </div>
      ...
      {showForm && (
        <div className="bg-[#241e14] border border-[#3a2e1e] rounded-xl p-5 space-y-4">
          <h3 className="font-semibold text-[#ddd5c5]">Nowy przychód</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div><label className="text-xs text-[#7a6a58] block mb-1">Kategoria *</label><select className="input w-full" value={form.category} onChange={e=>setForm(p=>({...p,category:e.target.value as IncomeCategory}))}>{categories.map(c=><option key={c.value} value={c.value}>{c.label}</option>)}</select></div>
            <div className="sm:col-span-2"><label className="text-xs text-[#7a6a58] block mb-1">Opis *</label><input className="input w-full" placeholder="np. Odsetki od lokaty bankowej" value={form.description} onChange={e=>setForm(p=>({...p,description:e.target.value}))} /></div>
            ...
            <div><label className="text-xs text-[#7a6a58] block mb-1">Data *</label><input className="input w-full" type="date" value={form.income_date} onChange={e=>setForm(p=>({...p,income_date:e.target.value}))} /></div>
          </div>
      ...
      <div className="bg-[#241e14] border border-[#3a2e1e] rounded-xl p-4 space-y-3">
        <div><p className="text-sm font-semibold text-[#b8a898]">📥 Import przychodów z CSV</p><p className="text-xs text-[#6a5a48] mt-0.5">Format: <code className="text-[#7a6a58]">data;opis;kategoria;kwota</code> (kategorie: odsetki, zwrot, dotacja, inne)</p></div>
        ...
        <div onDragOver={e=>{e.preventDefault();setCsvDragOver(true)}} onDragLeave={()=>setCsvDragOver(false)} onDrop={e=>{e.preventDefault();setCsvDragOver(false);const f=e.dataTransfer.files[0];if(f)handleCsvFile(f)}} onClick={()=>fileRef.current?.click()} className={`cursor-pointer rounded-xl border-2 border-dashed transition-all py-8 px-4 text-center select-none ${csvDragOver?'border-green-500 bg-amber-950/20':csvFileName?'border-green-700 bg-amber-950/20':'border-[#3a2e1e] bg-[#2a2218]/30 hover:border-[#4a3c28]'}`}>
      ...
      {tab==='summary'?(
        <div className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-[#241e14] border border-[#3a2e1e] rounded-xl p-4 text-center"><p className="text-xs text-[#6a5a48] mb-1">Wpłaty mieszkańców</p><p className="text-2xl font-bold text-amber-400">{pln(totalSettlements)}</p></div>
            <div className="bg-[#241e14] border border-[#3a2e1e] rounded-xl p-4 text-center"><p className="text-xs text-[#6a5a48] mb-1">Inne przychody</p><p className="text-2xl font-bold text-yellow-400">{pln(totalOther)}</p></div>
            <div className="bg-[#241e14] border border-[#3a2e1e] rounded-xl p-4 text-center"><p className="text-xs text-[#6a5a48] mb-1">Razem przychody</p><p className="text-2xl font-bold text-amber-400">{pln(totalAll)}</p></div>
          </div>

          <div className="bg-[#241e14] border border-[#3a2e1e] rounded-xl p-5">
            <h3 className="text-sm font-semibold text-[#7a6a58] mb-1">Przychody miesięcznie — {filterYear}</h3>
            <div className="flex gap-4 text-xs text-[#6a5a48] mb-4">
            ...
            <div className="bg-[#241e14] border border-[#3a2e1e] rounded-xl p-5">
              ...
              {(() => {
                ...
                return Object.entries(byCat).sort((a,b)=>b[1]-a[1]).map(([cat,amt])=>{
                  ...
                  return (<div key={cat} className="flex items-center gap-3 mb-3"><span className={`text-xs px-2 py-0.5 rounded-full w-32 text-center flex-shrink-0 ${catColors[cat]??catColors.inne}`}>{catLabel(cat)}</span><div className="flex-1 bg-[#2a2218] rounded-full h-2"><div className="bg-yellow-500 h-2 rounded-full" style={{width:`${pct}%`}}/></div><span className="text-sm font-semibold text-[#ddd5c5] w-28 text-right">{pln(amt)}</span></div>)
                })
              })()}
            </div>
          )}
        ...
        <div>
          {filtered.length === 0 ? (
            <div className="text-center py-16 text-[#6a5a48]"><p className="text-3xl mb-3">💰</p><p>Brak przychodów za {filterYear} rok.</p><button onClick={() => setShowForm(true)} className="mt-4 text-sm text-amber-400 hover:underline">+ Dodaj pierwszy przychód</button></div>
          ) : (
            <div className="space-y-2">
              {paginatedList.map(e => editId === e.id ? (
                <div key={e.id} className="bg-[#241e14] border border-amber-700 rounded-xl p-4 space-y-3">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => setEditId(null)} className="text-xs text-[#6a5a48] hover:text-[#b8a898]">Anuluj</button>
                  </div>
                </div>
              ) : (
                <div key={e.id} className="bg-[#241e14] border border-[#3a2e1e] rounded-xl px-4 py-3 flex items-center gap-3 flex-wrap hover:border-[#3a2e1e] transition">
                  ...
                  <div className="flex-1 min-w-0"><p className="text-sm font-medium text-[#ddd5c5] truncate">{e.description}</p><p className="text-xs text-[#6a5a48] mt-0.5">{new Date(e.income_date).toLocaleDateString('pl-PL')}{isSuperAdmin && ` · ${commMap[e.community_id] ?? '—'}`}</p></div>
              ...
              <div className="mt-4 pt-4 border-t border-[#3a2e1e] flex justify-between items-center"><p className="text-sm text-[#6a5a48]">{filtered.length} pozycji</p><p className="text-base font-bold text-amber-400">Razem: {pln(totalOther)}</p></div>
            </div>
          )}
        </div>
      )}
