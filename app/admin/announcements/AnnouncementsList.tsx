...
export default function AnnouncementsList({ announcements, communityMap, junctionMap, canEdit }: Props) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-1 bg-[#121c15] rounded-lg p-1 w-fit">
        {(['active', 'archive'] as const).map(t => (
          <button key={t} onClick={() => handleTabChange(t)}
            className={`px-4 py-1.5 rounded-md text-sm font-medium transition ${
              tab === t ? 'bg-[#0d1410] text-[#ecfdf5] shadow-lg shadow-black/30' : 'text-[#4d7a5f] hover:text-[#a7f3d0]'
            }`}>
            {t === 'active' ? 'Aktywne' : 'Archiwum'}
            <span className={`ml-1 text-xs font-semibold ${t === 'active' ? 'text-emerald-500' : 'text-[#6b9478]'}`}>
      ...
      {paged.length === 0 ? (
        <p className="text-sm text-[#6b9478]">{search ? 'Brak wyników.' : 'Brak ogłoszeń.'}</p>
      ) : (
        <div className="space-y-3">
          {paged.map((a) => {
            ...
            return (
              <div key={a.id} className={`bg-[#121c15] border rounded-xl p-4 transition ${
                tab === 'archive' ? 'opacity-70 border-[#162418]' :
                pinned ? 'border-emerald-700/60 shadow-sm shadow-emerald-900/20' : 'border-[#1e3324]'
              }`}>
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-0.5">
                      ...
                      <p className="font-semibold text-[#ecfdf5]">{a.title}</p>
                    </div>
                    <p className="text-sm text-[#4d7a5f] mt-1 line-clamp-2">{a.content}</p>
                    <div className="flex items-center gap-2 mt-2 flex-wrap">
                      <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${cls}`}>{text}</span>
                      {tab === 'archive' && (
                        <span className="text-xs px-2 py-0.5 rounded-full bg-[#1a1610] text-[#4d7a5f]">Archiwalne</span>
                      )}
                      <span className="text-xs text-[#6b9478]">{new Date(a.created_at).toLocaleDateString('pl-PL')}</span>
                      {a.end_date && (
                        <span className="text-xs text-[#6b9478]">do {new Date(a.end_date).toLocaleDateString('pl-PL')}</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    {canEdit && (
                      <button onClick={() => handleTogglePin(a)}
                        ...
                        className={`p-1.5 rounded-lg border transition ${
                          pinned
                            ? 'border-emerald-700/60 text-emerald-500 bg-emerald-950/30 hover:bg-emerald-950/50'
                            : 'border-[#162418] text-[#2a4a2a] hover:border-emerald-700/40 hover:text-emerald-600'
                        }`}>
                    ...
                    {canEdit && (
                      <Link href={`/admin/announcements/${a.id}`} className="text-sm text-emerald-500 hover:underline">
                        Edytuj
                      </Link>
                    )}
                  </div>
      ...
      {totalPages > 1 && (
        <div className="flex items-center gap-2 pt-2">
          ...
          <span className="text-sm text-[#4d7a5f]">{page} / {totalPages}</span>
