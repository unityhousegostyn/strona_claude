...
export default function AnnouncementsList({ announcements, communityMap, junctionMap, canEdit }: Props) {
  ...
  return (
    <div className="space-y-4">
        ...
        {(['active', 'archive'] as const).map(t => (
          <button key={t} onClick={() => handleTabChange(t)}
            className={`px-4 py-1.5 rounded-md text-sm font-medium transition ${
              tab === t ? 'bg-[#18140e] text-[#f0ebe0] shadow-lg shadow-black/30' : 'text-[#6a5a48] hover:text-[#b8a898]'
            }`}>
            {t === 'active' ? 'Aktywne' : 'Archiwum'}
            <span className={`ml-1 text-xs font-semibold ${t === 'active' ? 'text-amber-500' : 'text-[#7a6a58]'}`}>
      ...
      {paged.length === 0 ? (
        <p className="text-sm text-[#7a6a58]">{search ? 'Brak wyników.' : 'Brak ogłoszeń.'}</p>
      ) : (
        <div className="space-y-3">
          {paged.map((a) => {
            ...
            return (
              <div key={a.id} className={`bg-[#241e14] border rounded-xl p-4 transition ${
                tab === 'archive' ? 'opacity-70 border-[#2a2218]' :
                pinned ? 'border-amber-700/60 shadow-sm shadow-amber-900/20' : 'border-[#3a2e1e]'
              }`}>
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-2 flex-wrap">
                      ...
                      {tab === 'archive' && (
                        <span className="text-xs px-2 py-0.5 rounded-full bg-[#1a1610] text-[#6a5a48]">Archiwalne</span>
                      )}
                      <span className="text-xs text-[#7a6a58]">{new Date(a.created_at).toLocaleDateString('pl-PL')}</span>
                      {a.end_date && (
                        <span className="text-xs text-[#7a6a58]">do {new Date(a.end_date).toLocaleDateString('pl-PL')}</span>
                      )}
                    </div>
                  </div>
                      ...
                      <button onClick={() => handleTogglePin(a)}
                        ...
                        className={`p-1.5 rounded-lg border transition ${
                          pinned
                            ...
                            : 'border-[#2a2218] text-[#4a3c28] hover:border-amber-700/40 hover:text-amber-600'
                        }`}>
      ...
      {totalPages > 1 && (
        <div className="flex items-center gap-2 pt-2">
          ...
          <span className="text-sm text-[#6a5a48]">{page} / {totalPages}</span>
