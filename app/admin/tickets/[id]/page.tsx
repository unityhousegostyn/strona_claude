  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div className="flex items-center gap-3">
        <Link href="/admin/tickets" className="text-sm text-[#0f766e] hover:text-[#99f6e4]">← Zgłoszenia</Link>
      </div>

      <div className="bg-[#081918] border border-[#0f2d2a] rounded-xl p-6 space-y-4">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <h2 className="text-xl font-bold text-[#f0fdfa]">{ticket.title}</h2>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${
              ticket.status === 'open' ? 'bg-yellow-900/40 text-yellow-400' : 'bg-teal-900/40 text-teal-400'
            }`}>
        ...
        <p className="text-sm text-[#99f6e4] whitespace-pre-wrap">{ticket.description}</p>
        ...
        {attachmentUrl && (
          <a
            ...
            className="inline-flex items-center gap-2 text-sm text-teal-400 hover:underline bg-teal-950/40 px-3 py-1.5 rounded-lg"
          >
            📎 {attachmentName}
          </a>
        )}
      ...
      {history.length > 0 && (
        <div className="bg-[#081918] border border-[#0f2d2a] rounded-xl p-5 space-y-3">
          <h3 className="text-sm font-semibold text-[#115e59] uppercase tracking-wide">Historia</h3>
          <div className="space-y-2">
            {history.map((h, i) => (
              <div key={i} className="flex items-start gap-3 text-sm">
                <div className="w-1.5 h-1.5 rounded-full bg-[#133835] mt-2 flex-shrink-0" />
                <div>
                  <span className="text-[#99f6e4]">{actionLabel[h.action] ?? h.action}</span>
                  {h.action === 'toggle_ticket_status' && h.meta?.to && (
                    <span className="text-[#115e59]"> → <span className={h.meta.to === 'open' ? 'text-yellow-400' : 'text-teal-400'}>
