  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-[#ecfdf5]">Zgłoszenia</h2>
        <button
      ...
      <div className="relative">
        ...
        />
        {search && (
          <button onClick={() => handleSearch('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#4d7a5f] hover:text-[#a7f3d0] text-xs">✕</button>
        )}
      </div>

      {/* Zakładki */}
      <div className="flex gap-1 bg-[#121c15] rounded-xl p-1 w-fit">
        {(['open', 'closed'] as Tab[]).map(t => (
          <button
            key={t}
            onClick={() => handleTabChange(t)}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition ${
              tab === t ? 'bg-[#162418] text-[#ecfdf5] shadow-lg shadow-black/30' : 'text-[#4d7a5f] hover:text-[#a7f3d0]'
            }`}
          >
            {t === 'open' ? 'Aktywne' : 'Archiwum'}
            <span className={`text-xs font-semibold px-1.5 py-0.5 rounded-full ${
              t === 'open' ? 'bg-yellow-900/40 text-yellow-400' : 'bg-[#162418] text-[#6b9478]'
            }`}>
      ...
      {showForm && tab === 'open' && (
        <div className="bg-[#121c15] border border-[#1e3324] rounded-xl p-5 space-y-3">
          <h3 className="font-semibold text-[#d1fae5]">Nowe zgłoszenie</h3>
          {formError && (
          ...
          <div>
            <label className="block text-sm font-medium text-[#a7f3d0] mb-1">
              Załącznik <span className="text-[#6b9478] font-normal">(opcjonalnie, max 10 MB)</span>
            </label>
            <input
              type="file"
              onChange={e => setAttachment(e.target.files?.[0] ?? null)}
              className="block w-full text-sm text-[#6b9478] file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-emerald-950/40 file:text-emerald-400 hover:file:bg-emerald-900/40 transition"
            />
            {attachment && <p className="text-xs text-[#4d7a5f] mt-1">Wybrany: {attachment.name}</p>}
          </div>
          <div className="flex gap-3">
            <button onClick={handleSubmitTicket} disabled={isPending} className="bg-emerald-600 text-white text-sm font-semibold px-4 py-2 rounded-lg disabled:opacity-50">
      ...
      <div className="space-y-3">
        ...
        {paginated.map(t => (
          <div
            key={t.id}
            className={`bg-[#121c15] border rounded-xl p-4 flex items-start justify-between gap-4 cursor-pointer transition ${
              tab === 'closed' ? 'border-[#1e3324] opacity-75 hover:opacity-100 hover:border-[#1e3324]' : 'border-[#1e3324] hover:border-green-700'
            }`}
            onClick={() => router.push(`/admin/tickets/${t.id}`)}
          >
            <div className="min-w-0">
              <p className={`font-semibold truncate ${tab === 'closed' ? 'text-[#4d7a5f]' : 'text-[#ecfdf5]'}`}>{t.title}</p>
              <p className="text-sm text-[#4d7a5f] mt-1 line-clamp-1">{t.description}</p>
              <p className="text-xs text-[#6b9478] mt-2">
                {t.community?.name} · {new Date(t.created_at).toLocaleDateString('pl-PL')}
              </p>
            </div>
            <div className="flex flex-col items-end gap-2 flex-shrink-0">
              <span className={`text-xs font-medium px-2 py-1 rounded-full ${
                t.status === 'open' ? 'bg-yellow-900/40 text-yellow-400' : 'bg-emerald-900/40 text-emerald-400'
              }`}>
