  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-stone-900">Zgłoszenia</h2>
        <button
      ...
      <div className="relative">
        ...
        />
        {search && (
          <button onClick={() => handleSearch('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-stone-400 hover:text-stone-700 text-xs">✕</button>
        )}
      </div>
        ...
        {(['open', 'closed'] as Tab[]).map(t => (
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition ${
              tab === t ? 'bg-stone-200 text-stone-900 shadow-lg shadow-black/30' : 'text-stone-400 hover:text-stone-700'
            }`}
          >
            {t === 'open' ? 'Aktywne' : 'Archiwum'}
            <span className={`text-xs font-semibold px-1.5 py-0.5 rounded-full ${
              t === 'open' ? 'bg-yellow-900/40 text-yellow-400' : 'bg-stone-200 text-stone-500'
            }`}>
      ...
      {showForm && tab === 'open' && (
        <div className="bg-stone-100 border border-stone-200 rounded-xl p-5 space-y-3">
          <h3 className="font-semibold text-stone-800">Nowe zgłoszenie</h3>
          {formError && (
          ...
          <div>
            <label className="block text-sm font-medium text-stone-700 mb-1">
              Załącznik <span className="text-stone-500 font-normal">(opcjonalnie, max 10 MB)</span>
            </label>
            <input
              type="file"
              onChange={e => setAttachment(e.target.files?.[0] ?? null)}
              className="block w-full text-sm text-stone-500 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-green-950/40 file:text-green-400 hover:file:bg-green-900/40 transition"
            />
            {attachment && <p className="text-xs text-stone-400 mt-1">Wybrany: {attachment.name}</p>}
          </div>
      ...
      <div className="space-y-3">
        ...
        {paginated.map(t => (
          <div
            key={t.id}
            className={`bg-stone-100 border rounded-xl p-4 flex items-start justify-between gap-4 cursor-pointer transition ${
              tab === 'closed' ? 'border-stone-200 opacity-75 hover:opacity-100 hover:border-stone-200' : 'border-stone-200 hover:border-green-700'
            }`}
            onClick={() => router.push(`/admin/tickets/${t.id}`)}
          >
            <div className="min-w-0">
              <p className={`font-semibold truncate ${tab === 'closed' ? 'text-stone-400' : 'text-stone-900'}`}>{t.title}</p>
              <p className="text-sm text-stone-400 mt-1 line-clamp-1">{t.description}</p>
