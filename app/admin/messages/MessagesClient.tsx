    ...
    <div className="space-y-5">
      ...
      {isSuperAdmin && communities.length > 1 && (
        <div className="bg-[#081918] border border-[#0f2d2a] rounded-xl p-4">
          <label className="block text-xs font-semibold text-[#0f766e] mb-2 uppercase tracking-wide">Wspólnota</label>
          <select
            value={communityId}
      ...
      <div className="bg-[#081918] border border-[#0f2d2a] rounded-xl overflow-hidden">
        <div className="flex border-b border-[#0f2d2a]">
          {(['all', 'selected'] as const).map(m => (
            <button key={m} onClick={() => setMode(m)}
              className={`flex-1 py-3 text-sm font-semibold transition ${mode === m ? 'bg-[#051210] text-[#f0fdfa]' : 'text-[#115e59] hover:text-[#99f6e4]'}`}>
              {m === 'all'
        ...
        {mode === 'selected' && (
          <div className="p-4 space-y-3">
            ...
            />
            {loadingResidents ? (
              <p className="text-sm text-[#115e59] py-2">Ładowanie…</p>
            ) : filtered.length === 0 ? (
              <p className="text-sm text-[#115e59] py-2">Brak mieszkańców</p>
            ) : (
              <div className="space-y-1 max-h-64 overflow-y-auto pr-1">
                {/* Zaznacz wszystkich */}
                <label className="flex items-center gap-3 px-2 py-1.5 rounded-lg hover:bg-[#051210] cursor-pointer">
                  <input type="checkbox"
                    checked={filtered.length > 0 && filtered.every(r => selected.has(r.id))}
                    onChange={toggleAll}
                    className="w-4 h-4 rounded accent-teal-600"
                  />
                  <span className="text-xs font-semibold text-[#0f766e] uppercase tracking-wide">Zaznacz wszystkich</span>
                </label>
                <div className="border-t border-[#0c2220] my-1"/>
                {filtered.map(r => (
                  <label key={r.id} className="flex items-center gap-3 px-2 py-2 rounded-lg hover:bg-[#051210] cursor-pointer">
                    <input type="checkbox"
                    ...
                    <div className="min-w-0">
                      <p className="text-sm text-[#f0fdfa] font-medium truncate">{r.full_name ?? '—'}</p>
                      <p className="text-xs text-[#115e59] truncate">{r.email}</p>
                    </div>
                  </label>
                ))}
              </div>
        ...
        {mode === 'all' && (
          <div className="px-4 py-3 text-sm text-[#0f766e]">
            Wiadomość zostanie wysłana do <span className="font-semibold text-[#99f6e4]">{residents.length}</span> aktywnych mieszkańców.
          </div>
        )}
      </div>

      {/* Treść */}
      <div className="bg-[#081918] border border-[#0f2d2a] rounded-xl p-4 space-y-4">
        <div>
          <label className="block text-xs font-semibold text-[#0f766e] mb-2 uppercase tracking-wide">Temat *</label>
          <input
        ...
        <div>
          <label className="block text-xs font-semibold text-[#0f766e] mb-2 uppercase tracking-wide">Treść wiadomości *</label>
          <textarea
          ...
          />
          <p className="text-xs text-[#133835] mt-1 text-right">{body.length} znaków</p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        <p className="text-xs text-[#133835]">
          {recipientCount > 0
            ? <>Odbiorcy: <span className="text-[#0f766e] font-semibold">{recipientCount} {recipientCount === 1 ? 'osoba' : 'osób'}</span></>
            : <span className="text-[#0f2d2a]">Brak wybranych odbiorców</span>}
        </p>
        <button
          onClick={handleSend}
